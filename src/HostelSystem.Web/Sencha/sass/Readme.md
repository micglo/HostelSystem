...
folder are mapped to the application's root namespace, 'HostelSystem.Web'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "HostelSystem.Web"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'HostelSystem.Web' in the same way
as `"HostelSystem.Web/sass/src"`.
